...
- **`QGIS_DB_*`** — read-only подключение к `mggt_asu` на `localhost` для карты съёмки в модуле «Согласование». Обязательно при первом деплое `approval` (см. [approval_docs/PROD_DEPLOY.md](approval_docs/PROD_DEPLOY.md)).
