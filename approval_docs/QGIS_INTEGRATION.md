...
| `n_root` | `text[]` | да* | Список rootid соседних (смежных) паспортов, с которыми согласуют границу. |
| `v_root` | `text[]` | да* | «Кортеж» rootid — массив из **двух** текстовых значений, участвующих в согласовании, например `ARRAY['10482', '09811']`. |
| `name` | `text` | да* | Название/подпись согласования для отображения пользователю. |
| геометрия основного события | `geometry(Geometry, 4326)` | да | Контур или линия зоны согласования (основной чат на карте). SRID **4326** (WGS84). |
| `owners` | `text[]` | **да** | Список `OwnerLegalPersonId` организаций, которым видно это согласование в веб-интерфейсе. Без него запись создастся, но **никто её не увидит**. |

\* Поля `n_root`, `v_root`, `name` в схеме допускают `NULL`, но для рабочего согласования их нужно заполнять.

### Пример значений

```
incoming_guid = 2e333940-831b-48f5-9751-acd0c2880974   -- сгенерировать в QGIS
n_root        = ARRAY['09811']
v_root        = ARRAY['10482', '09811']
name          = Согласование границ паспорта ДТ-10482
owners        = ARRAY['10233594', '10233595']          -- OwnerLegalPersonId сторон
```
