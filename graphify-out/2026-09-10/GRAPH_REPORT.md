# Graph Report - GeoDjango  (2026-09-10)

## Corpus Check
- 233 files · ~1,120,351 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3939 nodes · 8719 edges · 247 communities (182 shown, 65 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 166 edges (avg confidence: 0.65)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `39a45d26`
...
6. `landing()` - 35 edges
7. `Approve` - 34 edges
8. `_valid_payload()` - 34 edges
9. `_post_qgis_approve()` - 33 edges
10. `_resolve_column_name()` - 32 edges
...
## Communities (247 total, 65 thin omitted)
...
Cohesion: 0.05
Nodes (84): addSignalTapeLayer(), applyGeometryToDossierGroup(), applyPopupHighlight(), autoRemoveIntersections(), bindCommentPointLayer(), bindDossierPolygonPopup(), bindPdfExportLink(), buildCommentPointPopupHtml() (+76 more)
...
Cohesion: 0.05
Nodes (68): home_page_config(), personal_page_config(), _annotate_and_filter_ods_registry_against_gis(), _annotate_kind_filter_membership(), _annotate_personal_ogh_statuses(), _annotate_personal_total_areas(), _attach_folded_ods_action_to_passport(), _build_home_page_context() (+60 more)

### Community 11 - "test_adjacent_relations.py"
Cohesion: 0.09
Nodes (39): _adjacent_layers_for_json_response(), _build_map_adjacent_dt_combined_sql(), _build_map_request_layer_branch(), _build_map_requests_sql(), _build_map_requests_sql_for_source(), _build_new_object_request_layer_branch(), _build_new_object_request_objects_sql(), _defer_map_context_layers() (+31 more)
...
Cohesion: 0.13
Nodes (23): _classify_object_kind(), _column_exists(), _compute_percents_for_object(), _gis_municipal_table_specs(), _insert_results_chunk(), _iter_gis_objects(), _quote_ident(), Daily batch: compute DGI intersection percents for all GIS passports/requests an (+15 more)
...
Cohesion: 0.18
Nodes (18): attachment_allowed_extensions(), attachment_max_bytes(), create_case_with_geometry(), create_event_from_adjacent(), _normalize_optional_string_list(), _normalize_string_list(), parse_geometry_payload(), _sync_primary_case_fields() (+10 more)
...
### Community 27 - "home"
Cohesion: 0.13
Nodes (24): geom_to_wgs84_sql(), Reproject MSC-77 geometry to WGS-84 using spatial_ref_sys.proj4text for source S, _flush_update_batch(), GeomSyncSpec, _iter_source_batches(), Any, Reproject MSC-77 (980077) geometries from mggt_asu into local geodb (EPSG:4326)., MSC-77 geom → WGS84 MultiPolygon (or Geometry) expression. (+16 more)
...
Cohesion: 0.07
Nodes (28): 1. Локально (из репозитория GeoDjango), 2. На сервере (обновление кода), 3. Проверка, 4. Вернуться к разработке, firewalld (RED OS / MGGT), Git на MGGT и токен развёртывания (hub.mos.ru), RED OS: Docker Hub недоступен, Безопасность (+20 more)
...
Cohesion: 0.11
Nodes (46): _add_calendar_months(), add_case_participant(), _approvals_progress(), attach_geometry_to_message(), attach_title_named(), build_case_title_named(), _case_approve_until(), _case_display_status() (+38 more)
...
Cohesion: 0.25
Nodes (18): _quote_ident(), _changed_predicate_sql(), _dry_run_keyed_counts(), _iter_qgis_batches(), KeyedSyncPlan, _merge_from_stage(), Full daily sync: mggt_asu (read-only) → geodb.  Keyed GIS tables: compare attrs, True when any sync attr or geom differs between target t and stage s. (+10 more)
...
Cohesion: 0.14
Nodes (39): get_accessible_approve(), build_geometries_feature_collection(), get_cases_queryset(), Short approve payload for QGIS list/detail headers., GeoJSON FeatureCollection for QGIS map layers., serialize_approve_qgis_summary(), qgis_api_host_allowed(), Access rules for the QGIS approval API (Host allowlist). (+31 more)
...
Cohesion: 0.18
Nodes (17): AuthenticationForm, add_demo_values(), copy_assets(), export(), extract_element(), main(), make_standalone(), modal_elements() (+9 more)
...
Cohesion: 0.06
Nodes (50): landing_page_config(), Page bootstrap config for approval templates (json_script)., _feature_select_sql(), build_layer_groups(), build_reference_layer_groups(), _empty_task_poly_meta(), format_survey_page_title(), layer_stack_order() (+42 more)

### Community 87 - "page_config.py"
Cohesion: 0.27
Nodes (12): add_recap_page_config(), _adjacent_nearby_meters_for_page(), build_page_config(), _defer_map_context_layers_for_page(), _editor_api_urls(), main_page_config(), map_deferred_layer_specs(), Page bootstrap config for pass_viewer map templates (json_script). (+4 more)
...
Cohesion: 0.15
Nodes (8): datetime, Django settings for pass_map project.  Generated by 'django-admin startproject', _obj_row_tuple(), Command, Remove export files under media/exports older than N days.  Intended for cron at, test_fetch_personal_master_details_queries_yardpoly(), test_fetch_personal_master_details_uses_grbs_for_odhpoly(), test_personal_object_details_maps_master_fields()
...
Cohesion: 0.13
Nodes (21): geometry_intersects_allowed_hood(), geometry_norm: GeoJSON geometry dict (not Feature/FC)., _auto_remove_square_table_name(), _comment_points_table_name(), _create_new_object(), _create_recap_object(), delete_comment_point(), _dgi_aprove_column_exists() (+13 more)
...
Cohesion: 0.23
Nodes (28): is_inspector_for_approve(), _actor_context(), api_add_case_participant(), api_approve_case(), api_case_detail(), api_change_case_owner(), api_create_adjacent_event(), api_create_case() (+20 more)
...
Cohesion: 0.11
Nodes (15): AppConfig, ApprovalConfig, ApproveAlreadyApprovedError, ApproveUserConflictError, Raised when upsert is attempted on a fully approved approve., Raised when upsert is attempted by a different user for the same incoming_guid., _sync_approve_status_after_case_approval(), _sync_approve_status_after_case_revoke() (+7 more)
...
Cohesion: 0.15
Nodes (29): addGeo(), applyModeToMap(), applyObjectToggle(), applySignalTape(), boot(), clearMapLayers(), csrfToken(), ensureMap() (+21 more)
...
Cohesion: 0.16
Nodes (8): BaseCommand, _column_exists(), Command, Delete GIS rows whose request_id is not in ods_request."BrId" and created_at is, _table_exists(), Command, Command, _to_text()
...
Cohesion: 0.10
Nodes (40): build_schema_feature_collection(), build_topopassport_feature_collection(), build_work_feature_collection(), _max_features(), Load approval map features from mggt_asu.work / topopassport schemas., GeoJSON polygon (EPSG:4326) of the survey object for the first matching guid., _should_clip_table_to_work(), _style_fields_for_table() (+32 more)
...
Cohesion: 0.06
Nodes (69): finalize_dgi_aprove_record(), Apply server-side username and timestamp defaults before DB write., EntryPointForm, add_object_page_config(), actions(), add_object(), _build_asu_ods_url(), _build_merge_allowed_sets() (+61 more)
...
Cohesion: 0.07
Nodes (63): get_accessible_approves(), get_accessible_cases_queryset(), get_owner_ids_for_username(), matching_case_owner_id(), _normalize_owner_ids_arg(), _normalized_participant_logins(), _owners_q_for_ids(), Access control for approval workflows. (+55 more)
...
Cohesion: 0.09
Nodes (26): api_bootstrap(), build_home_notification_events(), build_home_notifications(), enrich_cases_payload_title_named(), _finalize_notification_case_rows(), Batch-resolve names and attach title_named to serialized case list., Serialize approves with one batch poly-meta lookup for TaskGUIDs., Build home notifications payload.      Section 1: accessible non-approved Approv (+18 more)
...
### Community 209 - "navigateApp"
Cohesion: 0.08
Nodes (42): _append_auto_remove_mask_parts(), _append_intersection_mask_union_part(), auto_remove_intersections(), _auto_remove_mask_context(), _auto_remove_source_tokens(), _beskhoz_min_core_area_m2(), _beskhoz_min_half_width_m(), check_dgi_intersections() (+34 more)

### Community 215 - "views.py"
Cohesion: 0.22
Nodes (7): _notification_mine_approves_qs(), _notification_mine_cases_qs(), Approves personally assigned to login or with login as case participant., Cases on approves assigned to login or where login is a participant., forwards_backfill_roles(), Migration, Q
...
Cohesion: 0.50
Nodes (4): _mark_qgis_session_read_only(), Best-effort read-only; this module never issues writes on qgis., Return quoted master.<table> for bidregistry., _resolve_bidregistry_table()
...
## Knowledge Gaps
- **631 isolated node(s):** `Migration`, `Migration`, `Migration`, `Migration`, `Migration` (+626 more)
...
- **Why does `ExternalUser` connect `ExternalUser` to `test_approval_events.py`, `_remove_intersections_from_geometry`, `runSaveAndExportFlow`, `views.py`, `test_registration_request.py`, `page_config.py`, `test_approval_smoke.py`, `landing`, `initOwnedMap`, `AppConfig`, `models.py`, `hood_scope.py`, `test_dgi_intersection_results.py`, `add_recap`, `work_layer_label`, `geojson_dynamic.py`, `refreshAutoRemoveModalOptions`, `Command`?**
