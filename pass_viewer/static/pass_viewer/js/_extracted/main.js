if (L && L.drawLocal) {
        ...
        function buildObjectPopup(
            properties,
            fallbackRootid = '-',
            fallbackName = '-',
            fallbackRequestId = '-',
            fallbackCustomerLegalPersonId = '-',
            fallbackDepartmentLegalPersonId = '-',
            fallbackCustomerLegalPersonName = '-',
            fallbackDepartmentLegalPersonName = '-'
        ) {
            const rootid = properties?.rootid ?? fallbackRootid;
            const name = properties?.name ?? fallbackName;
            const requestId = properties?.request_id ?? fallbackRequestId;
            const ownerLegalPersonId = properties?.owner_legal_person_id ?? '-';
            const ownerLegalPersonName = properties?.owner_legal_person_name ?? '-';
            const customerLegalPersonId = properties?.customer_legal_person_id ?? fallbackCustomerLegalPersonId;
            const departmentLegalPersonId = properties?.department_legal_person_id ?? fallbackDepartmentLegalPersonId;
            const customerLegalPersonName = properties?.customer_legal_person_name ?? fallbackCustomerLegalPersonName;
            const departmentLegalPersonName = properties?.department_legal_person_name ?? fallbackDepartmentLegalPersonName;
        ...
        function buildPdfIntersectionPopupHtml(properties) {
            ...
            if (src === 'ДГИ') {
                ...
                return (
                    ...
                    (!descrText || ['null', 'none', '-'].includes(descrText.toLowerCase())
                        ? ''
                        : '<div style="margin-top: 6px;"><strong>Кадастровый номер:</strong> ' + escapeHtml(descr) + '</div>') +
                    (!addressText || ['null', 'none', '-'].includes(addressText.toLowerCase())
                        ? ''
                        : '<div style="margin-top: 6px;"><strong>Адрес:</strong> ' + escapeHtml(address) + '</div>') +
            ...
            if (src === 'ОДХ' || src === 'ОЗН') {
                ...
                return (
                    ...
                    (!rootidText || ['null', 'none', '-'].includes(rootidText.toLowerCase())
                        ? ''
                        : '<div style="margin-top: 6px;"><strong>Паспорт №:</strong> ' + escapeHtml(rootid) + '</div>') +
                    (!nameText || ['null', 'none', '-'].includes(nameText.toLowerCase())
                        ? ''
                        : '<div style="margin-top: 6px;"><strong>Название:</strong> ' + escapeHtml(name) + '</div>') +
                    ((src === 'ОЗН')
                        ? (hasOwner
                            ? '<div style="margin-top: 6px;"><strong>Балансодержатель:</strong> ' + escapeHtml(ownerDisplay) + '</div>'
                            : '')
                        : (hasCustomer
                            ? '<div style="margin-top: 6px;"><strong>Балансодержатель:</strong> ' + escapeHtml(customerDisplay) + '</div>'
                            : '')) +
                    (hasDepartment
                        ? '<div style="margin-top: 6px;"><strong>Отраслевой ОИВ:</strong> ' + escapeHtml(departmentDisplay) + '</div>'
                        : '') +
                    buildPopupMetaFieldsHtml(properties) +
                    '</div>'
                );
            }
            return buildObjectPopup(properties);
        }
        ...
        function bindPopupToLayer(layer, popupHtml) {
            if (!layer) {
                return;
            }
            layer.eachLayer((item) => item.bindPopup(popupHtml));
        }
        ...
        const selectedRowCtid = "{{ selected_ctid|default:''|escapejs }}";
        const effectiveEntryRequestId = "{{ effective_request_id|default:''|escapejs }}";
        const selectedCustomerLegalPersonId = "{{ selected_customer_legal_person_id|default:''|escapejs }}";
        const selectedDepartmentLegalPersonId = "{{ selected_department_legal_person_id|default:''|escapejs }}";
        const selectedCustomerLegalPersonName = "{{ selected_customer_legal_person_name|default:''|escapejs }}";
        const selectedDepartmentLegalPersonName = "{{ selected_department_legal_person_name|default:''|escapejs }}";
        const selectedStartdate = "{{ selected_startdate|default:''|escapejs }}";
        const selectedDatesurvey = "{{ selected_datesurvey|default:''|escapejs }}";
        const selectedCreatetype = "{{ selected_createtype|default:''|escapejs }}";
        const selectedPassportMetaProps = {
            startdate: selectedStartdate,
            datesurvey: selectedDatesurvey,
            ...
            if (addCommentPointButton) {
                if (!rid) {
                    ...
                    addCommentPointButton.title =
                        'Нет номера заявки — укажите его при входе на страницу или в карточке объекта.';
                } else {
        ...
        function refreshLayerPanelCounts() {
            const counts = {
                selected: countGroupFeatures(selectedGroup),
        ...
        renderRecapsLayer(parseGeometryData('recaps-geometry-data'));
        ...
        if (mergedAdjacentDtInitial) {
            L.geoJSON(mergedAdjacentDtInitial, {
        ...
        const requestObjectsGeoInitial = normalizeGeoJson(requestObjectsGeometry);
        if (requestObjectsGeoInitial) {
            const selectedRootidTextInit = (selectedRootid || '').trim();
            const filteredRequestsInitial =
                selectedRootidTextInit && requestObjectsGeoInitial.type === 'FeatureCollection'
                    ? {
                          ...requestObjectsGeoInitial,
                          features: requestObjectsGeoInitial.features.filter((feature) => {
                              const rid = String(feature?.properties?.rootid ?? '').trim();
                              return rid !== selectedRootidTextInit;
                          }),
                      }
                    : requestObjectsGeoInitial;
            L.geoJSON(filteredRequestsInitial, {
        ...
        if (selectedGeo) {
            ...
            const popupHtml = buildObjectPopup(
        ...
        function askSnapRadiusMeters(currentValue) {
            ...
            if (!Number.isFinite(parsed) || parsed <= 0) {
                window.alert('Радиус должен быть положительным числом.');
                return null;
            }
            return parsed;
        }
        ...
        snapToggle.addEventListener('change', () => {
            ...
            if (snappingEnabled) {
                const pickedRadius = askSnapRadiusMeters(snapDistanceMeters);
                ...
                snapDistanceMeters = pickedRadius;
        ...
        function buildCurrentGeometry() {
            ...
            const baseGeometries = source
                .map((feature) => feature.geometry)
                .filter((geometry) => geometry && (geometry.type === 'Polygon' || geometry.type === 'MultiPolygon'));
            if (baseGeometries.length === 1) {
                return baseGeometries[0];
            }
            if (baseGeometries.length > 1) {
        ...
        function countPolygonLikeGeometriesInCollection(geojsonCollection) {
            ...
            features.forEach((feature) => {
                ...
                if (geom.type === 'Polygon') {
                    ...
                    count += (geom.coordinates || []).length || 1;
                }
            });
            return count;
        }
        function hasNewPolygonBeyondSelected() {
            if (!isEditing) {
                return false;
            }
            const baselineCount = countPolygonLikeGeometriesInCollection(selectedEditableGeo);
            const currentCount = countPolygonLikeGeometriesInCollection(editableGroup.toGeoJSON());
            return currentCount > baselineCount;
        }
        function extractPolygonGeometriesFromCollection(geojsonCollection) {
            const features = geojsonCollection?.features || [];
            const polygons = [];
            features.forEach((feature) => {
                const geom = feature?.geometry;
        ...
        function toGeometryFromPolygonList(polygons) {
        ...
        function buildNewGeometryBeyondSelected() {
            if (!isEditing) {
                return null;
            }
            const baselineCount = countPolygonLikeGeometriesInCollection(selectedEditableGeo);
            const currentPolygons = extractPolygonGeometriesFromCollection(editableGroup.toGeoJSON());
            if (currentPolygons.length <= baselineCount) {
                return null;
            }
            return toGeometryFromPolygonList(currentPolygons.slice(baselineCount));
        }
        function toIntersectionGeometry(raw) {
        ...
        function renderRelationLayers(layers) {
            ...
            parsed.intersects = excludeSelectedRootid(parsed.intersects);
            parsed.touches = excludeSelectedRootid(parsed.touches);
            parsed.nearby = excludeSelectedRootid(parsed.nearby);
        ...
        async function checkRelations() {
            ...
            const hasNewPolygon = hasNewPolygonBeyondSelected();
                    ...
                    body: JSON.stringify(
                        (() => {
                            const payload = {
                                ...
                                selected_geometry: selectedGeometryForCheck,
                                geometry_for_selected_check: newGeometryForSelectedCheck,
                            };
        ...
        function openSaveModal() {
            ...
            if (newObjectNameInput.value) {
                ...
                setTimeout(() => newObjectNameInput.focus(), 0);
            }
        }
        ...
        async function saveObjectToDb(geometryToSave, name, requestId) {
            const savePayload = {
        ...
        async function fetchIntersectsLayerForPdfExport(geometry) {
            ...
            const selectedGeometryForCheck =
                hasNewPolygon && typeof toIntersectionGeometry === 'function'
                    ? toIntersectionGeometry(selectedGeometry)
                    : null;
            const newGeometryForSelectedCheck =
                hasNewPolygon && typeof buildNewGeometryBeyondSelected === 'function'
                    ? buildNewGeometryBeyondSelected()
                    : null;
            const body = {geometry, source_label: selectedSourceLabel};
            if (selectedGeometryForCheck != null) {
                body.selected_geometry = selectedGeometryForCheck;
            }
            if (newGeometryForSelectedCheck != null) {
                body.geometry_for_selected_check = newGeometryForSelectedCheck;
            }
            const entryRid = String(effectiveEntryRequestId || '').trim();
            if (entryRid) {
                body.request_id = entryRid;
            }
            const response = await fetch("{% url 'check_new_object_relations' %}", {
                method: 'POST',
                headers: {
