(function () {
    ...
    const pointInRing2D = PV.pointInRing2D.bind(PV);
    ...
    const cross2d = PV.cross2d.bind(PV);
    const pointOnSegment2d = PV.pointOnSegment2d.bind(PV);
    const segmentsIntersect2d = PV.segmentsIntersect2d.bind(PV);
    const findBestParentPolygonIndex = PV.findBestParentPolygonIndex.bind(PV);
    const tagEditablePartsAfterLineCut = PV.tagEditablePartsAfterLineCut.bind(PV);
    ...
    async function exportAllParts() {
        ...
        try {
            for (let g = 0; g < requestIdsSorted.length; g += 1) {
                ...
                const saveResponse = await fetch(cfg.urls.saveNewObject, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRFToken': getCookie('csrftoken') || '' },
                    body: JSON.stringify({
                        geometry: geometry,
