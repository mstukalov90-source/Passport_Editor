...
def _hood_owner_geom_union_sql_and_params(cursor, owner_legal_person_id) -> tuple[str | None, list[Any]]:
    """
...
def get_hood_allowed_districts_geojson(cursor, owner_legal_person_id) -> dict:
    ...
    if ratio <= 0:
        ...
        sql = (
            "WITH " + _hood_owner_union_cte_sql(union_sql) + "SELECT jsonb_build_object("
            "'type', 'FeatureCollection', "
            "'features', COALESCE(jsonb_agg("
            f"jsonb_build_object("
            f"'type', 'Feature', "
...
def resolve_hood_scope_for_owner(cursor, owner_legal_person_id) -> dict:
    ...
    if ratio <= 0:
        ...
        sql = (
            "WITH " + _hood_owner_union_cte_sql(union_sql) + f"SELECT ST_AsText(ST_UnaryUnion(ST_Collect(h.{hg}))) "
            f"FROM {_hq_quote_ident(hood_table)} h "
            "CROSS JOIN owner_u ou CROSS JOIN owner_area oa "
            f"WHERE ou.g IS NOT NULL AND NOT ST_IsEmpty(ou.g) "
            f"AND ST_Intersects(h.{hg}, ou.g) "
...
def bind_hood_scope(mode: str, wkt: str | None):
...
def resolve_and_bind_hood_scope(request):
    ...
    try:
        ...
        return
    row = (
        ExternalUser.objects.filter(login=user.username)
        .only(
